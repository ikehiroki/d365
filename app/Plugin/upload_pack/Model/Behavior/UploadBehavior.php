...
class UploadBehavior extends ModelBehavior {
    ...
    private function _fetchFromUrl($url) {
        ...
        $slash = '/';
        $data['name'] = end(explode($slash, $url));
        $dot = '.';
        $data['tmp_name'] = tempnam(sys_get_temp_dir(), $data['name']) . $dot . end(explode($dot, $url));
        ...
        $semicolon = ';';
        $data['type'] = reset(explode($semicolon, $response['header']['Content-Type']));
